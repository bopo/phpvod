      ...
      <eq name="k" value="1">
        <div class="tab-pane fade in active" id="{$feifei.player_name_en}">
          <ul class="list-unstyled text-center play-list">
            ...
            <else/>
              <li class="col-sm-2 col-xs-4"><a href="{:ff_url_vod_play($list_id,$list_dir,$vod_id,$vod_ename,$feifei['player_sid'],$pid)}" class="btn btn-default btn-sm" id="{$vod_id}_{$feifei.player_sid}_{$pid}" target="{$target}">{$feifeison.title}</a></li>
            </eq>
            </volist>
          </ul>
        </div>
