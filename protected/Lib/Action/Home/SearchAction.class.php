...
class SearchAction extends HomeAction{
	public function vod(){
		...
		if($infos){
			foreach($infos as $key=>$value){
				$infos[$key]['vod_link'] = ff_url_vod_read($value['list_id'],$value['list_dir'],$value['vod_id'],$value['vod_ename'],$value['vod_jumpurl']);
			}
	...
	public function news(){
		...
		if($infos){
			foreach($infos as $key=>$value){
				$infos[$key]['news_link'] = ff_url_news_read($value['list_id'],$value['list_dir'],$value['news_id'],$value['news_ename'],$value['news_jumpurl']);
			}
			$this->ajaxReturn($infos,"ok",1);
		}else{
			$this->ajaxReturn($infos,"error",0);
