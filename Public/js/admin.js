...
function setbind(event, cid, bind){
	...
	$.ajax({
		url: '?s=Admin-Cj-Setbind-cid-'+cid+'-bind-'+bind,
		cache: false,
		async: false,
...
var submitbind = function (cid,bind){
	$.ajax({
		url: '?s=Admin-Cj-Insertbind-cid-'+cid+'-bind-'+bind,
		success: function(res){
...
//dialog
function ff_dialog($strPath,$title){
	$('#ff_dialog_title').html($title);
	$('#ff_dialog_body').html('<iframe src="'+$strPath+'" width="100%" height="95%" frameborder="0" scrolling="auto" style="overflow-x:hidden;"></iframe>');
	$("#ff-dialog-back").show();
	$("#ff-dialog-box").show();
}
