...
class CacheAction extends BaseAction
{
	...
	//清除系统缓存AJAX 并不会删除S函数的缓存数据
  	public function del()
  	{
		...
		$this->unlink('./Runtime/~app.php');
		$this->unlink('./Runtime/~runtime.php');
	...
	public function delhtml()
	{
		...
		if('index' == $id)
		{
			$this->unlink(HTML_PATH.'index'.C('html_file_suffix'));
		}
		elseif('vodlist'== $id)
		{
			if(is_dir(HTML_PATH.'Vod_show'))
				$dir->delDir(HTML_PATH.'Vod_show');
		}
		elseif('vodread' == $id)
		{
			if(is_dir(HTML_PATH.'Vod_read'))
				$dir->delDir(HTML_PATH.'Vod_read');    
		}
		elseif('vodplay' == $id)
		{
			if(is_dir(HTML_PATH.'Vod_play'))
				$dir->delDir(HTML_PATH.'Vod_play');  
		}
		elseif('news_list' == $id)
		{
	...
	public function delhtml_day()
	{
		...
		if($array)
		{
			foreach($array as $key=>$val)
			{
			  	...
			  	$this->unlink('./Html/Vod_read/'.$id);
				$this->unlink('./Html/Vod_play/'.$id);
			}
		  	
		  	import("ORG.Io.Dir");
			$dir = new Dir;
			
			if(!$dir->isEmpty('./Html/Vod_show'))
				$dir->delDir('./Html/Vod_show');

			if(!$dir->isEmpty('./Html/Ajax_show'))
				$dir->delDir('./Html/Ajax_show');
			
			$this->unlink('./Html/index'.C('html_file_suffix'));						
		}
